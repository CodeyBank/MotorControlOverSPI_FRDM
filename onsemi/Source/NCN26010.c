...
typedef void (*NCNCB_t)(uint32_t uwtick);
...
{
	//PRINTF("Data ready callback called\n");
	dataReady = 1;
	if (errorOccurred > 0)
	{
		errorOccurred -= 1;
...
uint32_t ConfigureMACPHY()
{
	...
	if (USE_INTERNAL_MAC_ADDRESS)
	{
		...
		for(int i=0; i<SIZE_OF_MAC_ADDR; i++){
			PRINTF("MAC Address: ");
			PRINTF("%d ", g_currentNodeMACAddr[i]);
			PRINTF("\n");
		}
...
void ConvertEndianness(uint32_t valueToConvert, uint32_t *convertedValue)
{
	...
	memcpy(convertedBytes, (char *)&valueToConvert, variableSize); /** cast and copy an uint32_t to a uint8_t array **/
	...
	memcpy((uint8_t *)convertedValue, convertedBytes, variableSize); /* copy the swapped convertedBytes to an uint32_t */
}
...
//		PRINTF("Waiting for data in loop ... \n");
		...
		if (dataReady == 0 && !errorOccurred)
		{
			...
			//do {
			//	vTaskDelay(1000);
			//} while ((GPIO_PinRead(NCN_IRQ_BASE, NCN_IRQ_PIN) != 0) && (dataReady == 0));
			//vTaskDelay(1);
		}
...
void T1S_InterpretEthernetTypeFrame(uint8_t *receiveMACPHYBuff, stEthernetFrame_t stVarEthernetFrame)
{
	...
	if (messageType == MESSAGETYPE_SEND)
	{
		...
		for (uint8_t configCount = 0; configCount < numberOfConfigToDo; configCount++)
		{
			...
			//PRINTF("Received Type of config is %d and MMS %d and Address 0x%04X\n", typeOfConfig,  receiveMACPHYBuff[receivedDataBuffIndex],(uint16_t)receiveMACPHYBuff[receivedDataBuffIndex+1]);
			...
			else if (typeOfConfig == CONFIGTYPE_REGWRITE)
			{
				...
				regWriteValue = (uint32_t)((receiveMACPHYBuff[receivedDataBuffIndex + 3] << 24) | (receiveMACPHYBuff[receivedDataBuffIndex + 2] << 16) | (receiveMACPHYBuff[receivedDataBuffIndex + 1] << 8) | receiveMACPHYBuff[receivedDataBuffIndex]);
	...
	else if (messageType == MESSAGETYPE_RESPONSE)
	{
		...
		for (uint8_t configCount = 0; configCount < numberOfConfigToDo; configCount++)
		{
			...
			if (typeOfConfig == CONFIGTYPE_REGREAD)
			{
				...
				regValue = (uint32_t)((receiveMACPHYBuff[receivedDataBuffIndex + 3] << 24) | (receiveMACPHYBuff[receivedDataBuffIndex + 2] << 16) | (receiveMACPHYBuff[receivedDataBuffIndex + 1] << 8) | receiveMACPHYBuff[receivedDataBuffIndex]);
				receivedDataBuffIndex += EACH_REG_SIZE;
